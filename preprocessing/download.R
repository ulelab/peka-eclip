...
# sbatch -N 1 -t 12:00:00 --array 1-1374%100 -J eclip -o logs/%A_%a.log --wrap="Rscript --vanilla download.R"
...
# metadata.dt <- fread("~/Ule/entropy/eclip/metadata.tsv")
...
metadata.dt[, output := paste0(`Biosample term name`, "-",
                               ...
                               `Paired end`, ".fastq.gz")]
download.file(metadata.dt$`File download URL`[i], destfile = file.path("data", paste0(i, "_", metadata.dt$output[i])), method = "wget", quiet = FALSE)
